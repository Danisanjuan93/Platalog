    ...
    public function match($pathinfo)
    {
        ...
        if (0 === strpos($pathinfo, '/api')) {
            ...
            if (0 === strpos($pathinfo, '/api/users')) {
                ...
                // api_get_users_workers
                if (preg_match('#^/api/users/(?P<id>[^/]++)/workers$#s', $pathinfo, $matches)) {
                    if (!in_array($this->context->getMethod(), array('GET', 'HEAD'))) {
                        $allow = array_merge($allow, array('GET', 'HEAD'));
                        goto not_api_get_users_workers;
                    }

                    return $this->mergeDefaults(array_replace($matches, array('_route' => 'api_get_users_workers')), array (  '_controller' => 'ApiBundle\\Controller\\UsersController::getWorkersAction',  '_format' => 'json',));
                }
                not_api_get_users_workers:
